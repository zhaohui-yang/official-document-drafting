...
- **症状**：导出的 `.docx` 在别的电脑上字体变了样。
  ...
  **对策**：在目标机器先 `bash scripts/install_fonts.sh` 安装 `assets/fonts/` 字体；需要锁定最终视觉时在已装字体的机器上导出 PDF。
...
```bash
python3 adapters/skill/build.py --check                       # 校验 SKILL.md/dist/模板是否与 prompts/ 主源同步
python3 -m pytest -q                                          # 跑全部测试（含主源同步与 references 一致性守卫）
python3 scripts/generate_docx.py --doc-type 通知 --show-font-plan    # 查看某文种实际落到的字体、字号、文件
python3 scripts/generate_docx.py --doc-type 通知 --show-layout-plan  # 仅查看该文种的版式参数
```
