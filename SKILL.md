...
- 正文通常用 3 号仿宋体，首行缩进 2 字符；在 Markdown 或纯文本成稿中，正文自然段前默认保留两个全角空格，体现“首行缩进两格”的常见写法；导出 Word 时仍按首行缩进 2 字符处理，不依赖半角空格堆砌版式。固定行距和段前后距由当前文种绑定的 `layout_profile` 统一约束，正式公文默认正文行距为 `580 twips`（约 `29.00pt`），阅读型内部材料通常为 `600 twips`（`30pt`）。
