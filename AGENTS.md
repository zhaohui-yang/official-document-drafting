...
- 文种的「撰写思路」写在各自 `spec.md` 的可选 `## 撰写思路` 段（思路为核心、结构可调），由 `src/adapters/shared.py`（已拆分为 paths/profiles/doc_types/rendering 四个模块，shared.py 为兼容再导出层）注入产物；不要把思路散写进散文或代码。
...
- 除上述情形外，只要能并行尝试多种不同方案、假设、来源、输入场景或实现路径，或能把资料检索、代码阅读、分析、改写、验证等动作并行执行，也可以主动开启多个 agents；能并行试错、并行对照或并行执行的部分都优先并行。
