...
其中，`adapters/` 不是单纯为离线场景准备的目录，而是“面向不同消费端的适配层”：

- `adapters/skill/`：把 `prompts/` 主源适配成在线 skill / agent 可直接使用的产物
- `adapters/webui/`：把 `prompts/` 主源适配成 WebUI、AnythingLLM、Qwen、Claude.ai 等提示词宿主可直接使用的产物
...
- `adapters/`：把同一套 `prompts/` 主源适配到不同运行环境的适配层，不只服务离线场景
- `adapters/skill/build.py`：在线 skill 适配层，生成 `SKILL.md`、`agents/openai.yaml` 等在线产物
- `adapters/webui/build.py`：离线提示词适配层，生成 WebUI / Qwen / AnythingLLM / Claude.ai 等宿主可直接使用的提示词
