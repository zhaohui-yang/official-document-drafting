...
## 当前文种撰写思路
```markdown
- 起笔：目的＋依据式，「为……，根据……，结合……实际，现就……提出如下意见」。
- 谋篇：规范性意见固定骨架「总体要求（指导思想＋基本原则＋工作目标）— 主要任务 — 保障措施 — 组织实施」；目标带时间节点与量化指标，基本原则写成排比式导向句，主要意见按「是什么—怎么办—谁来办」组织。
- 语域（中央·部委基准）：政论笔法，定调段讲政治站位、提总体要求，部署段回到平实可执行。
- 结尾：上行报批用 `以上意见如无不妥，请批转执行。`；下行部署用 `请结合实际认真贯彻执行。`
- 误区：与通知混淆（通知重部署、意见重原则办法）；上行误用请示语气；下行缺可操作措施沦为空泛表态。
```
