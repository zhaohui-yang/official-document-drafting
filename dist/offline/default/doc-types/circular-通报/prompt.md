...
## 当前文种撰写思路
```markdown
- 起笔：缘由式导语点出事件及其关注度，「近期，……」。
- 谋篇：基本情况 → 评价分析 → 工作要求；无论表彰还是批评，都要有事实与数据支撑，再逐条剖析。
- 语域（下行）：定性准确、分寸得当，表彰不浮夸、批评不情绪化。
- 结尾：表彰/批评类常用 `特此通报。`；情况类用 `现提出以下工作要求……` 或 `希望各地区各部门引以为戒。`（与本文种「写作规则」口径一致）
- 误区：与知照性通知混淆；事迹或问题空泛、无数据支撑。
```
