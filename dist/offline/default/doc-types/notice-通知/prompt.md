...
- 未定的机关名、文号、日期留占位，不编造。
- 典型取材来源：单机参考本单位职责清单与既有制度；联网查上级发文、会议精神与被转发/印发件原文（.gov.cn）。
```

## 当前文种撰写思路
```markdown
- 主送：高规格下行通知主送常是多行机关序列（「各省、自治区、直辖市人民政府办公厅，国务院各部委、各直属机构：」），不必收成单一机关。
- 起笔：部署型用目的式「为……，现就……通知如下」，缘由式可引具体依据（上级会议、文件号）「……要求，……经……同意，现就有关事项通知如下」；印发／转发型用依据式「《……》已经……，现予印发」。
- 谋篇：下行部署可「先总体要求、再分条任务、最后工作要求」逐层加压；但这只是一种写法——中央部委通知也大量按工作事项／环节平行分条（一、二、…、五），末条收束于「意义＋总要求／抓落实」，不必硬套三段式。每条把责任、时限、报送、标准尽量写实。知照性事项直叙即可，不必层层套用部署结构。
