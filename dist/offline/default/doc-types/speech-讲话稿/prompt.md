...
## 当前文种撰写思路
```markdown
- 起笔：称谓只用一次「同志们：」，点明场合与目的，「下面，我讲几点意见」。
- 谋篇：「怎么看 — 怎么办 — 怎么干」递进；形势判断段用政论笔法，部署段用「一要、二要、三要」。
- 语域（中央·部委基准）：书面为主、口语为辅；观点句式大标题对仗工整、提气有力；数据为论点服务，不罗列数字。
- 结尾：总结升华＋提气收束，「最后，我强调几点」。
- 误区：空话套话口号化（「正确的废话」）；偏离主旨与领导意图；层级滥用、逻辑跳跃。
```
