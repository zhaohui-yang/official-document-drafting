...
- 一函一事。
- 答复类先给来文标题与发文字号。
- 典型取材来源：以来函原件与本单位办理意见为准；联网查对方机关职能与相关政策（.gov.cn）。
```

## 当前文种撰写思路
```markdown
- 起笔：去函用「因……工作需要，现将有关事项函告如下」；商洽型常用「为……，现商请你们协助……」「为……，现将有关事项函商如下」；属答复（复函）的先引来文再给结论，如「你们关于……的来函收悉。经……同意，现函复如下」「您提出的关于……的提案（建议）收悉。经商……，现答复如下」。
- 谋篇：一函一事，事项清楚；不相隶属或平行机关之间商洽、询问、答复、请求批准均用函。商洽/请批型把请求事项、所需配合、反馈方式与时限写清；答复/复函型把答复结论置前，再分条说明。事项较多时正文可按「一、二、……」平行分条（如商洽函常分专业领域、对方要求、推荐或办理程序、联系方式）。
- 主送：可为单一机关，也可为多行平行/不相隶属机关序列（部委商洽函常并列「发展改革委、教育部、科技部……各省、自治区、直辖市……」等多家收文机关），按从中央到地方、从主到次排列。
- 语域（平行）：身份平等，称「贵单位／你们」，既不居高临下也不仰视，措辞礼貌得体；商洽用「商请」「拟请」，不用命令式部署口吻。
- 结尾：商洽/请求型用 `特此函达。`／`特此函商。`／`请予支持为盼。`；复函用 `特此函复。`；提案、建议答复型常以「感谢您对……工作的关心和支持」收束，不强求「特此函复」。
- 误区：把本应用函的事项升格为请示或通知（隶属关系请示用「请示」，下行部署用「通知」，不相隶属或平行机关之间则用函）；一函多事；答复型把结论埋在文末让对方通篇寻找。
```
